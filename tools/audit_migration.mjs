...
function classify(mountName, facetSlugs, kindSlugs, invMentions) {
  ...
  return { facetHit, kindHit, invHit };
}
...
function main() {
  ...
  const rows = mounts.map((m) => {
    const c = classify(m, facets, kinds, invMentions);
    let status;
    if (c.facetHit && c.invHit) status = "DONE";
  ...
  if (update) {
    ...
    console.log(`[audit] inventory updated. ${tally.DONE || 0}/${total} subsystems have facet+doc coverage.`);
    process.exit(0);
  }
  ...
  console.log("");
  for (const r of rows) {
    const mark = r.status === "DONE" ? "OK   " : r.status === "FACET" ? "FACET" : r.status === "DOC" ? "DOC  " : "MISS ";
...
function rewriteInventoryAuditBlock(rows, tally, total) {
  ...
  const lines = [
    ...
    "",
    `**Coverage:** ${tally.DONE || 0} DONE / ${tally.FACET || 0} FACET-only / ${tally.DOC || 0} DOC-only / ${tally.MISSING || 0} MISSING — total ${total} mount* subsystems.`,
    "",
    "| Mount call | facet hit | kind hit | inv hit | status |",
    "|---|---|---|---|---|",
    ...rows.map((r) => `| \`${r.mount}\` | ${r.facetHit ? "yes" : "—"} | ${r.kindHit ? "yes" : "—"} | ${r.invHit ? "yes" : "—"} | ${r.status} |`),
    "",
    end,
  ];
