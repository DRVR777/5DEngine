...
import {
  ...
  mountDropSpawner, WEAPON_DROP_MAP as _WEAPON_DROP_MAP, GameProgress, mountEnemyRobotEmpTick, mountEnemyHeavyGrenadeTick, mountEnemyBossRockTick, mountEnemyPoisonerSpitTick, mountEnemyIncendiaryTick, mountEnemyBossSlamTick, mountEnemyPoisonerRangedSpitTick, mountEnemyFastChargeTick, mountEnemySniperTick, mountEnemyStrafeMeleeTick, mountEnemyRobotPlasmaTick, mountEnemyAiScaffoldTick,
  // Combat
  ...
  // Social
  createLanSession, mountMpBadge, mountAppMultiplayerWiring, mountRuntimeErrorReporter, mountLoadCheckOverlay,
  // Core
  Core, mountEngineRegistry, mountEcsBootstrap, mountBuilderUiRefresh, mountWorldBuilderControls, mountWorldBuilderHotbar, mountWorldScreens, mountDeviceGraphWiring,
  // Config
...
function releasePointerForModal() {
  if (typeof document !== "undefined" && document.pointerLockElement && document.exitPointerLock) {
    document.exitPointerLock();
  }
}
function requestGameplayPointer() {
  const _isBlocked = () =>
    document.getElementById("perkPicker")?.style?.display !== "none" ||
    document.getElementById("difficultyScreen")?.style?.display === "flex" ||
    computerOpen || computerEntering || buildMode || _heroDead;
  if (_isBlocked() || typeof renderer === "undefined") return;
  const canvas = renderer.domElement || document.querySelector("canvas");
  if (!canvas || document.pointerLockElement) return;
  // Defer by 120ms — browsers reject requestPointerLock if called immediately
  // after exitPointerLock (Promise rejection: "lock lost"). Re-check blockers
  // at call time so a perk picker or difficulty screen that appeared in that
  // window doesn't get locked over.
  setTimeout(() => {
    if (document.pointerLockElement || _isBlocked()) return;
    try {
      const _p = canvas.requestPointerLock();
...
const _robotPlasmaTick         = mountEnemyRobotPlasmaTick({ THREE, scene, enemyBullets, actions: { playSfx } });
const _strafeMeleeTick         = mountEnemyStrafeMeleeTick({ ARMOR_ABSORB, getHeroPos: () => world.players.get("hero"), getEnemyPos: id => world.players.get(id), resolveMove: (mover, du, dv) => GTAPhysics.resolveAABBMove(mover, du, dv, buildingBlockers), setEnemyPos: (id, x, z, rot, u, v) => world.setPlayer(id, x, z, rot, u, v), applyStatusEffect: effect => { if (typeof StatusEffects !== "undefined") StatusEffects.apply("hero", effect); }, actions: { playSfx, flashDamage, screenShake: _applyScreenShake, setHeroHp: v => { heroHp = v; }, setHeroArmor: v => { heroArmor = v; }, setHeroLastDamageT: v => { heroLastDamageT = v; }, setDmgDir: (ang, until) => { _dmgDirAngle = ang; _dmgDirUntil = until; }, setHeroKb: (u, v, t) => { _heroKbU = u; _heroKbV = v; _heroKbT = t; }, setHeroFireT: v => { _heroFireT = Math.max(_heroFireT, v); }, onHeroDeath: () => { if (!_heroDead) _heroShowDeathScreen(); }, emitHeroDied: id => { if (typeof EventBus !== "undefined") EventBus.emit(EventBus.EVENTS.HERO_DIED, { killer: id }); } } });
const _enemyAiScaffold         = mountEnemyAiScaffoldTick({ getEnemyPos: id => world.players.get(id), setEnemyPos: (id, a, b, c, u, v) => world.setPlayer(id, a, b, c, u, v), resolveMove: (mover, du, dv) => GTAPhysics.resolveAABBMove(mover, du, dv, buildingBlockers), hasLOS: (u0, v0, u1, v1) => hasLOS(u0, v0, u1, v1, buildingBlockers), makeBt: opts => typeof BehaviorTree !== "undefined" ? BehaviorTree.makeEnemyTree(opts) : null, runBt: (bt, ctx) => { if (typeof BehaviorTree !== "undefined") BehaviorTree.run(bt, ctx); }, pathfind: (u0, v0, u1, v1) => typeof AStar !== "undefined" ? AStar.findPath(u0, v0, u1, v1) : null, getAlertBroadcastT: () => _alertBroadcastT, setAlertBroadcastT: t => { _alertBroadcastT = t; }, robotPlasmaTick: _robotPlasmaTick, empTick: _empTick, heavyGrenadeTick: _heavyGrenadeTick, bossRockTick: _bossRockTick, poisonerSpitTick: _poisonerSpitTick, incendiaryTick: _incendiaryTick, bossSlamTick: _bossSlamTick, poisonerRangedSpitTick: _poisonerRangedSpitTick, fastChargeTick: _fastChargeTick, sniperTick: _sniperTick, strafeMeleeTick: _strafeMeleeTick, getHeroDead: () => _heroDead, getGodMode: () => typeof Engine !== "undefined" && Engine.debug.godMode, getDodgeT: () => _dodgeT, getHeroHp: () => heroHp, getHeroArmor: () => heroArmor, actions: { playSfx, showToast, spawnParticles: _spawnParticles } });
...
let _tickDriveMode = false; // when true, tick() skips rAF scheduling (used by test bridge tickGame)
function tick() {
  ...
  if (computerOpen) {
    ...
    if (!_tickDriveMode) requestAnimationFrame(tick);
    return;
  }
...
// ═══ EXTRACTED → src/systems/bullet_physics_tick.js (iter 681) ══════════════════════
  // Pseudocode: substep loop (5×), enemy hit, pierce, world hit → all in bullet_physics_tick.js
  _bulletPhysicsTick.tick(dt, { nowMs: now });
...
// EXTRACT-PLAN see iter 527 note above — enemy AI loop starts here (~800 lines)
  ...
  for (const en of enemies) {
    ...
    if (!en.dead) {
      _enemyAiScaffold.tick(dt, en, { heroU: _heroNowAI.u, heroV: _heroNowAI.v, nowMs: now, em, crouching, smokeZones, heroShotAlertT: _heroShotAlertT, heroShotAlertU: _heroShotAlertU, heroShotAlertV: _heroShotAlertV }); // → enemy_ai_scaffold_tick.js
    }
    // tickEntry handles both alive visual updates and dead fall animation — must run for dead enemies too
